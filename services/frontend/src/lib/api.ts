import type { ApiErrorResponse, IssueTone, RuntimeIssue } from "@/types/analysis";

const errorMessageByCode: Record<string, string> = {
  audio_upload_required: "Please upload an audio file before starting the analysis.",
  ...
  analysis_not_ready: "This analysis is still running. Wait for completion and try again.",
  analysis_result_not_found: "The analysis finished, but its result file could not be loaded.",
  replay_request_invalid: "The replay request is invalid. Check the JSON file path and try again.",
...
export async function parseApiError(response: Response, fallbackPrefix: string): Promise<RuntimeIssue> {
  ...
  const tone = payload?.code ? toneByCode[payload.code] || "destructive" : response.status >= 500 ? "destructive" : "warning";
  const meta = {
    code: payload?.code,
