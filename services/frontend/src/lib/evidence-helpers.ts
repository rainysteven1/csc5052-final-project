...
export function buildNodeVisuals(result: AnalysisStateResult | null): NodeVisual[] {
  ...
  const errors = Array.isArray(normalizedResult.errors) ? normalizedResult.errors : [];
  ...
  return [
    {
      ...
      title: truncate(sanitizeDisplayText(normalizedResult.transcript, "No transcript"), 48),
    ...
    },
    {
      ...
      title: truncate(getCoachingSummary(resultPayload, coaching, judgment), 56),
...
export function buildNodeDetails(
  ...
  const currentSegments = Array.isArray(current?.segments) ? current.segments : [];
  const currentRawAsrSegments = Array.isArray(current?.raw_asr_segments) ? current.raw_asr_segments : [];
  ...
  if (node === "input") {
    ...
    return {
      title: "Input stage",
      summary: sanitizeDisplayText(snapshot.transcript || currentTranscript, "--"),
      stats: [
        { label: "Format", value: String(audio.format || "n/a") },
        { label: "ASR mode", value: String(snapshot.asr_mode || currentMeta.asr_mode || "n/a") },
      ...
      bullets: [
        ...
        ...segments.slice(0, 2).map((segment) => `${segment.segment_id}: ${truncate(String(segment.text || ""), 72)}`),
      ],
    };
  }
  ...
  if (node === "evidence") {
    ...
    return {
      ...
      summary: "Rule-driven signals are extracted and grouped into a unified evidence payload.",
      stats: [
        ...
        { label: "Evidence segments", value: String(evidenceSummary.segment_count || 0) },
      ],
      bullets: [
        ...((Array.isArray(evidenceSummary.dominant_dimensions) ? evidenceSummary.dominant_dimensions : []).map((item) => `dimension: ${String(item)}`)),
        ...((Array.isArray(evidenceSummary.top_lexical_triggers) ? evidenceSummary.top_lexical_triggers : []).slice(0, 2).map((item) => {
          const row = asRecord(item) || {};
          return `top lexical: ${String(row.label || "n/a")} ×${String(row.count || 0)}`;
        })),
        ...((Array.isArray(evidenceSummary.top_disfluency_patterns) ? evidenceSummary.top_disfluency_patterns : []).slice(0, 2).map((item) => {
          const row = asRecord(item) || {};
          return `top disfluency: ${String(row.label || "n/a")} ×${String(row.count || 0)}`;
        })),
        ...lexical.flatMap((row) => asStringArray(row.triggers)).slice(0, 4),
        ...issues.slice(0, 4).map((issue) => `${String(issue.type)} · ${String(issue.text)} ×${String(issue.count ?? 1)}`),
  ...
  if (node === "coaching") {
    ...
    return {
      ...
      bullets: [
        ...
        ...focusItems,
        ...strengths,
        ...asStringArray(judgment.risk_segments).map((item) => `risk segment: ${item}`),
        ...feedback.slice(0, 2).map((row) => `${String(row.segment_id)} · ${truncate(String(row.rewrite || row.problem || ""), 72)}`),
      ].slice(0, 6),
    };
  }
  ...
  return {
    ...
    stats: [
      ...
      { label: "Warnings", value: String((Array.isArray(snapshot.warnings) ? snapshot.warnings : currentWarnings).length) },
      { label: "Errors", value: String((Array.isArray(snapshot.errors) ? snapshot.errors : currentErrors).length) },
