...
export const useAnalysisStore = create<AnalysisStore>((set, get) => ({
  ...
  setReplayCursor: (index) => {
    ...
    set((state) => {
      ...
      return {
        ...
        activeNode: nextEvent?.node ? normalizeNodeName(nextEvent.node) : state.activeNode,
  ...
  selectNode: (node) => {
    ...
    const matchedEvent = [...events].reverse().find((event) => event.node === node);
    const nodePayload =
      finalState
        ? {
  ...
  submitLiveRun: async () => {
    ...
    if (!audioFile) {
      set({ error: makeRuntimeIssue("Please select an audio file first.", "warning") });
      return;
    }
    ...
    try {
      ...
      if (!nextJob.events_url) {
        set({ error: makeRuntimeIssue("Missing events URL for live run.", "warning"), isSubmitting: false });
        return;
      }
      ...
      const handleEvent = (rawEvent: MessageEvent<string>) => {
        try {
          ...
          set((current) => ({
            ...
            activeNode: parsed.node ? normalizeNodeName(parsed.node) : current.activeNode,
          ...
          if (parsed.event_type === "analysis_failed") {
            set({
              error: makeRuntimeIssue(parsed.message || "Analysis failed.", "destructive", {
                code: typeof parsed.payload?.code === "string" ? parsed.payload.code : undefined,
                requestId: parsed.request_id || (typeof parsed.payload?.request_id === "string" ? parsed.payload.request_id : undefined),
                traceId: parsed.trace_id || (typeof parsed.payload?.trace_id === "string" ? parsed.payload.trace_id : undefined),
              }),
            });
            source.close();
            liveEventSource = null;
          ...
          set({ error: makeRuntimeIssue("Failed to parse an SSE event from the backend.", "warning") });
        }
      };
      ...
      source.onerror = () => {
        set({
          error: makeRuntimeIssue("SSE connection dropped. Check whether the backend is still running.", "warning"),
          isSubmitting: false,
        });
        source.close();
        liveEventSource = null;
  ...
  loadReplay: async () => {
    ...
    try {
      ...
      if (!normalizedResult) {
        throw makeRuntimeIssue("Replay payload did not include a valid result state.", "destructive");
      }
      ...
      set({
        ...
        replayCursor: 0,
        isReplayPlaying: false,
        activeNode: firstEvent?.node ? normalizeNodeName(firstEvent.node) : "input",
      ...
      set({
        error:
          typeof replayError === "object" && replayError !== null && "message" in replayError
