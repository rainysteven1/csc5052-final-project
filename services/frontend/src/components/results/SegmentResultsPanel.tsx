...
export function SegmentResultsPanel({
  selectedId = null,
  onSelect,
}: SegmentResultsPanelProps) {
  ...
  const lexicalById = new Map(
    (Array.isArray(agentOutputs?.lexical) ? agentOutputs.lexical : []).map(
      (row) => [String(row.segment_id || ''), row]
    )
  );
  const prosodyById = new Map(
    (Array.isArray(agentOutputs?.prosody) ? agentOutputs.prosody : []).map(
      (row) => [String(row.segment_id || ''), row]
    )
  );
  const disfluencyById = new Map(
    (Array.isArray(agentOutputs?.disfluency)
      ? agentOutputs.disfluency
      : []
    ).map((row) => [String(row.segment_id || ''), row])
  );
  ...
  return (
    <PageSectionCard
      ...
      title='Segment index'
      action={
        <Badge variant='outline'>{summary.segmentResults.length} rows</Badge>
      }
      ...
      {summary.segmentResults.length === 0 ? (
        ...
        <div className='space-y-3'>
          ...
          {summary.segmentResults.map((segment) => {
            ...
            const disfluency =
              asRecord(disfluencyById.get(segment.segment_id)) || {};
            const drillRows = [
              lexical.interpretation
                ? ['Lexical', String(lexical.interpretation)]
                : null,
              prosody.interpretation
                ? ['Prosody', String(prosody.interpretation)]
                : null,
              disfluency.interpretation
                ? ['Disfluency', String(disfluency.interpretation)]
                : null,
            ...
            return (
              <SegmentResultCard
                ...
                drillRows={drillRows}
                selected={Boolean(
                  selectedId && segment.segment_id === selectedId
                )}
                onSelect={onSelect}
              />
            );
