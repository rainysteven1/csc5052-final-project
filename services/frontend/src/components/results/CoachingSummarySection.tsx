...
export function CoachingSummarySection({ summary }: CoachingSummarySectionProps) {
  return (
    <ResultSectionCard
      title="Overall coaching synthesis"
      action={
        <div className="flex flex-wrap gap-2">
          <Badge variant="outline">{summary.coachingProvider || "deterministic fallback"}</Badge>
          <Badge variant="outline">{summary.coachingModel || "n/a"}</Badge>
        </div>
      }
    >
      <div className="grid gap-3 lg:grid-cols-2">
        <ResultInfoBlock label="Why the pipeline landed here" value={summary.summary} />
        <ResultInfoBlock
          label="Practice direction"
          value={summary.coachingFocus[0] || summary.dominantCauses[0] || "No explicit follow-up direction captured."}
