...
export function ResultsLensPanel({
  ...
  const activeFeedback =
    summary.feedbackRows.find((row) => String(row.segment_id || "") === activeFeedbackId) ||
    summary.feedbackRows[0] ||
    null;
  const activeSegment =
    summary.segmentResults.find((segment) => segment.segment_id === activeSegmentId) ||
...
function buildCardsByView(
  ...
  return {
    ...
    feedback: [
      {
        label: "Selected card",
        value: activeFeedback ? String(activeFeedback.segment_id || "segment") : "none",
        meta: activeFeedback ? String(activeFeedback.severity || "unknown") : "no active card",
      },
      {
        label: "Focus tags",
        value: activeFeedback ? String(asStringArray(activeFeedback.focus_tags).length) : "0",
        meta: activeFeedback ? asStringArray(activeFeedback.focus_tags)[0] || "no tags" : "no tags",
      },
      {
        label: "Practice steps",
        value: activeFeedback ? String(asStringArray(activeFeedback.practice_steps).length) : "0",
        meta: activeFeedback ? String(activeFeedback.practice || "no practice note") : "no practice note",
      },
    ],
    segments: [
      {
        ...
        meta: activeSegment ? `${activeSegment.token_count || 0} tokens` : "no active segment",
      },
      {
        label: "Final score",
        value: typeof activeSegment?.scores?.final === "number" ? activeSegment.scores.final.toFixed(3) : "n/a",
        meta: activeSegment?.text ? activeSegment.text.slice(0, 48) : "no segment text",
      },
      {
        label: "Linked coaching",
        value: activeFeedbackId || activeSegmentId || "none",
        meta: activeFeedback ? String(activeFeedback.problem || activeFeedback.reason || "no linked coaching note") : "no linked coaching note",
      },
    ],
  };
}
