...
export function SegmentSpotlightPanel({
  selectedId = null,
  fallbackToFirst = true,
}: SegmentSpotlightPanelProps) {
  ...
  const feedbackRow =
    summary.feedbackRows.find(
      (row) => String(row.segment_id || '') === selectedSegment?.segment_id
    ) || null;
  const evidenceReadings = getSegmentEvidenceReadings(
    finalState,
    selectedSegment?.segment_id || null
  );
    ...
    <PageSectionCard
      ...
      action={
        <Badge variant='outline'>
          final {selectedSegment?.scores?.final?.toFixed(3) || '--'}
        </Badge>
      }
        ...
        <div className='space-y-4'>
          ...
          >
            ...
            <SegmentLinkedCoachingGrid
              ...
              practice={String(
                feedbackRow?.practice || 'No linked practice note.'
              )}
            />
          </ResultSectionCard>
...
function getSelectedSegment(
  ...
  const matchedSegment =
    segments.find((segment) => segment.segment_id === selectedId) || null;
...
function getSegmentEvidenceReadings(
  ...
  return [
    buildEvidenceReading(
      ...
      'No lexical interpretation.'
    ),
    buildEvidenceReading(
      'Prosody',
      prosody.interpretation,
      'No prosody interpretation.'
    ),
    buildEvidenceReading(
      'Disfluency',
...
function buildEvidenceReading(label: string, value: unknown, fallback: string) {
  return {
    label,
...
function findAgentOutputRow(rows: unknown, segmentId: string | null) {
  return (
    asRecord(
      (Array.isArray(rows) ? rows : []).find(
        (row) =>
          String((row as Record<string, unknown>)?.segment_id || '') ===
          segmentId
      )
    ) || {}
  );
