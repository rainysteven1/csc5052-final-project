...
export function FeedbackDeck({
  ...
  const lexicalById = new Map(
    lexicalRows.map((row) => [String(row.segment_id || ''), row])
  );
  const prosodyById = new Map(
    prosodyRows.map((row) => [String(row.segment_id || ''), row])
  );
  const disfluencyById = new Map(
    disfluencyRows.map((row) => [String(row.segment_id || ''), row])
  );
  ...
  return (
    <PageSectionCard
      ...
      action={
        <Badge variant='outline'>{summary.feedbackRows.length} cards</Badge>
      }
        ...
        <div className='space-y-3'>
          ...
          {summary.feedbackRows.map((row, index) => {
            ...
            const evidenceSources = [
              ...
              prosody.interpretation || prosody.coaching_hint
                ? 'Prosody'
                : null,
              disfluency.interpretation || disfluency.practice_hint
                ? 'Disfluency'
                : null,
            ...
            return (
              <FeedbackDeckCard
                ...
                selected={Boolean(
                  selectedId && String(row.segment_id) === selectedId
                )}
                onSelect={onSelect}
              />
            );
