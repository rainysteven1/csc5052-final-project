...
export function FeedbackSpotlightPanel({
  selectedId = null,
  fallbackToFirst = true,
}: FeedbackSpotlightPanelProps) {
  ...
  const selectedRow = getSelectedFeedbackRow(
    summary.feedbackRows,
    selectedId,
    fallbackToFirst
  );
  const emptyTitle =
    !fallbackToFirst && selectedId
      ? 'No linked coaching card'
      : 'No coaching feedback';
  const focusTags = selectedRow ? asStringArray(selectedRow.focus_tags) : [];
  const practiceSteps = selectedRow
    ? asStringArray(selectedRow.practice_steps)
    : [];
  ...
  return (
    <PageSectionCard
      ...
      action={
        <SeverityBadge
          severity={
            typeof selectedRow?.severity === 'string'
          ...
          >
            <div className='grid gap-3 lg:grid-cols-2'>
              <ResultInfoBlock
                label='Problem'
                value={String(
                  selectedRow.problem || selectedRow.reason || 'n/a'
                )}
              />
              <ResultInfoBlock
                label='Rewrite'
                value={String(selectedRow.rewrite || 'n/a')}
            ...
            <ResultInfoBlock
              label='Practice'
              value={String(selectedRow.practice || 'n/a')}
              className='mt-3'
            />
            ...
            <div className='mt-3'>
              <ResultChipList
                items={practiceSteps}
                emptyLabel='No practice steps captured.'
              />
            </div>
          </ResultSectionCard>
        </div>
