...
export function TimelinePanel() {
  ...
  const selectTimelineEvent = useAnalysisStore(
    (state) => state.selectTimelineEvent
  );
  ...
  return (
    ...
    >
      <div className='space-y-3'>
        {timeline.length === 0 ? (
          <EmptyState
            title={mode === 'live' ? 'No timeline events' : 'No replay events'}
          />
        ) : (
          timeline.map((event) => {
                ...
                <div className='flex items-start justify-between gap-3'>
                  <div>
                    ...
                    <div className='mt-1 text-xs text-muted-foreground'>
                      ...
                      {prettifyNode(event.node)}
                      {substep ? ` / ${prettifyNode(substep)}` : ''} ·{' '}
