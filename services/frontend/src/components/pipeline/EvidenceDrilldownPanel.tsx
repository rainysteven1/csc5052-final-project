...
export function EvidenceDrilldownPanel() {
  ...
  const insightGroups = buildInsightGroups(lexicalRows, prosodyRows, disfluencyRows);
      ...
      rows: lexicalRows
        ...
        .map((row) => ({
          ...
          lines: collectInsightLines([
            ...
            typeof row.rewrite_hint === "string" ? `Rewrite hint: ${row.rewrite_hint}` : null,
            typeof row.practice_hint === "string" ? `Practice hint: ${row.practice_hint}` : null,
          ]),
    ...
    },
    {
      ...
      rows: prosodyRows
        ...
        .map((row) => ({
          ...
          lines: collectInsightLines([
            typeof row.interpretation === "string" ? row.interpretation : null,
            typeof row.coaching_hint === "string" ? `Coaching hint: ${row.coaching_hint}` : null,
            typeof row.feedback_focus === "string" ? `Feedback focus: ${row.feedback_focus}` : null,
          ]),
...
function EvidenceInsightGroupCard({
  group,
}: {
  group: EvidenceInsightGroup;
}) {
  return (
