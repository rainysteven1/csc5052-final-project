...
export function EventDetailPanel() {
  ...
  const activeEvent =
    mode === 'replay'
      ? events[replayCursor] || null
      : events.length
        ? events[events.length - 1]
        : null;
  const payload =
    asRecord(activePayload) || asRecord(activeEvent?.payload) || {};
    ...
    >
      {!activeEvent ? (
        ...
        <>
          <EventMetaStrip
            ...
            progressLabel={
              activeEvent.progress != null
                ? `${Math.round(activeEvent.progress * 100)}%`
                : '--'
            }
            requestId={requestId}
            traceId={traceId}
          ...
          <PipelineInfoBlock
            label='Message'
            value={activeEvent.message || 'No runtime message captured.'}
