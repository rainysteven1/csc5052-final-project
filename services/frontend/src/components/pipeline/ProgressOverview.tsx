...
export function ProgressOverview() {
  ...
  const activeSubstep = typeof payload.substep === "string" ? payload.substep : null;
  ...
  return (
    <PageSectionCard eyebrow="Runtime" title="Execution strip" className="h-full" contentClassName="flex min-h-0 flex-1 flex-col gap-4">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)] xl:items-start">
        <div className="grid gap-4">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            ...
            <RuntimeTile
              label="Event flow"
              value={mode === "live" ? `${events.length}` : `${events.length ? replayCursor + 1 : 0}/${events.length}`}
              detail={mode === "live" ? "captured events" : "visible frame / total"}
            />
            <RuntimeTile
              label="Latest event"
              value={latest?.event_type ? prettifyNode(latest.event_type) : "Awaiting stream"}
              detail={latest?.message || (mode === "live" ? "No SSE event received yet." : "Load a replay file to populate the runtime timeline.")}
            />
            <RuntimeTile label="Analysis ID" value={job?.analysis_id || "n/a"} detail={job?.audio_filename || "No audio attached"} />
            <RuntimeTile
              label="Trace link"
              value={job?.request_id || job?.trace_id || "n/a"}
              detail={job?.trace_id ? `trace ${job.trace_id}` : "Trace metadata will appear once the backend reports it."}
            />
          </div>
        </div>

        {mode === "replay" ? <ReplayControls /> : <LiveRuntimePanel latestMessage={latest?.message || null} latestTime={latest?.created_at || null} />}
      </div>
    </PageSectionCard>
  );
}
