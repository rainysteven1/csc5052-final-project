    ...
    <>
      <div className="grid gap-4 xl:grid-cols-2">
        <PipelineSectionBlock label="Lexical triggers">
          <SpotlightChipList items={lexical.flatMap((row) => asStringArray(row.triggers)).slice(0, 10)} />
        ...
        <PipelineSectionBlock label="Disfluency issues">
          <SpotlightChipList
            items={disfluency
              ...
              .slice(0, 8)
              .map((issue: Record<string, unknown>) => `${String(issue.type)} · ${String(issue.text)}`)}
          />
        </PipelineSectionBlock>
      </div>
      ...
      <PipelineSectionBlock label="Context">
        <div className="space-y-3">
          ...
          <SurfaceCalloutList items={asStringArray(context.style_constraints).slice(0, 3)} />
        </div>
      </PipelineSectionBlock>
...
function averageScore(rows: Array<Record<string, unknown>>) {
  const scores = rows.map((row) => (typeof row.score === "number" ? row.score : 0)).filter(Number.isFinite);
  if (!scores.length) {
    return 0;
  }
