...
export function EvidenceSpotlight({
  ...
  const segments = Array.isArray(snapshot.segments)
    ...
    : Array.isArray(current?.segments)
      ? current.segments
      : [];
  const topLexical = Array.isArray(evidenceSummary.top_lexical_triggers)
    ? evidenceSummary.top_lexical_triggers
    : [];
  const riskSegments = Array.isArray(evidenceSummary.risk_segments)
    ? evidenceSummary.risk_segments
    : [];
  const scenario =
    typeof current?.scenario === 'string' ? current.scenario : 'presentation';
  ...
  const evidenceInsights = [
      ...
      .map((row) => ({
        ...
        lines: [
          ...
          typeof row.rewrite_hint === 'string'
            ? `Rewrite: ${row.rewrite_hint}`
            : null,
          typeof row.practice_hint === 'string'
            ? `Drill: ${row.practice_hint}`
            : null,
    ...
    ...prosody
      .filter(
        (row) =>
          row && (row.interpretation || row.coaching_hint || row.feedback_focus)
      )
      .slice(0, 2)
      .map((row) => ({
    ...
    ...disfluency
      .filter(
        (row) =>
          row && (row.interpretation || row.practice_hint || row.feedback_focus)
      )
      .slice(0, 2)
      .map((row) => ({
