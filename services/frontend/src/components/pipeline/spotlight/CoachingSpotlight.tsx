...
export function CoachingSpotlight({
  ...
  const judgment =
    asRecord(snapshot.judgment) || asRecord(currentAgentOutputs.judgment) || {};
  const coaching =
    asRecord(snapshot.coaching) || asRecord(currentAgentOutputs.coaching) || {};
  ...
  const resultPayload =
    asRecord(snapshot.result) || asRecord(currentResult) || {};
    ...
    <div className='space-y-4'>
      ...
      <div className='grid gap-4 xl:grid-cols-2'>
        <PipelineSectionBlock label='Dominant causes'>
          <SpotlightChipList
            items={asStringArray(resultPayload.dominant_causes)}
          />
        </PipelineSectionBlock>
        ...
        {feedback.slice(0, 4).map((row) => (
          ...
          >
            ...
            <div className='grid gap-3 lg:grid-cols-2'>
              <PipelineInfoBlock
                label='Problem'
                value={String(row.problem || row.reason || 'n/a')}
                tone='tone-secondary-soft'
              />
              <PipelineInfoBlock
                label='Rewrite'
                value={String(row.rewrite || 'n/a')}
